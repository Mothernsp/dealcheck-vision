...
// Never throws: results are only available once a batch has ended, so on any
// error (e.g. called before the batch ended) it returns whatever it has and the
// caller falls back to synchronous processing for the missing deals.
export async function collectResults(client, batchId) {
  ...
  try {
    ...
    console.warn(`  could not read batch results (${batchId}): ${err.message}`);
  }
  return out;
}
