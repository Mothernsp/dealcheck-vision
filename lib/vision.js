...
export async function classifyAllDocuments(files) {
  ...
  const res = await getClient().messages.create({
    model: HAIKU,
    // 16000 handles ~30 documents at ~500 output tokens each.
    // A 39-page combined scan can contain 10-15 distinct documents.
    max_tokens: 16000,
    system: [
      {
        type: 'text',
  ...
  logCacheUsage(`classify:${files.length}files`, res.usage);
  ...
  // Results may contain MORE entries than input files — a single uploaded PDF
  // may produce multiple document objects if it contained several docs.
  // Normalise: ensure source_file is set (fall back to positional match).
  return results.map((r, i) => {
    const sourceFile =
      r.source_file ||
      r.filename || // backwards compat
