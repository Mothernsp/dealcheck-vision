...
export async function processDeal(dealId, orgId) {
  ...
  try {
    ...
    // Dump Claude's classifications so we can spot wrong guesses. This includes
    // customer PII (names, VINs, addresses), so it is OFF by default and only
    // emitted when DEBUG_VISION_DUMP=true — never in production logs.
    if (process.env.DEBUG_VISION_DUMP === 'true') {
      console.log('\n========== VISION DUMP ==========');
    ...
    // Step 4: compliance check. The deterministic comparison engine resolves
    // cross-document discrepancies; the source scans are passed so the model can
    // verify any flagged value against the actual document.
    ...
    const report = await runComplianceCheck(perFile, { overrides, images });
