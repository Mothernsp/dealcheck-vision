...
// Can this mime type be sent as a vision block at all? Used to skip
// unrenderable grounding scans instead of throwing on them.
function canVisionRender(mimeType) {
...
export function buildClassifyParams(files, { model = CLASSIFY_MODEL } = {}) {
  ...
  return {
    model,
    // Sonnet 4.6 caps output at 64K. Large multi-doc deals can produce
    // classification JSON past 16K; the caller streams (messages.stream) so a
    // cap this high doesn't risk an SDK HTTP timeout.
    max_tokens: 64000,
...
export async function classifyAllDocuments(files, { model = CLASSIFY_MODEL, onUsage } = {}) {
  ...
  // Stream so the 64K max_tokens cap can't trip an SDK HTTP timeout on large
  // deals; getFinalMessage() returns the same shape as messages.create.
  const stream = getClient().messages.stream(buildClassifyParams(files, { model }));
  ...
  if (images) {
    ...
    for (const c of comparisons) {
      ...
      for (const op of c.operands) {
        ...
        if (!img) continue;
        seen.add(sf);
        // A discrepant document whose scan can't be rendered (unknown/exotic
        // mime) is skipped, not fatal — grounding is best-effort, and the
        // comparison results still ship without it.
        if (!canVisionRender(img.mimeType)) {
          console.warn(`compliance grounding: skipping unrenderable scan ${sf} (${img.mimeType})`);
          continue;
        }
