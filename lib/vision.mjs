...
export function buildComplianceParams(perFile, { model = COMPLIANCE_MODEL, overrides = '' } = {}) {
...
export async function runComplianceCheck(perFile, { model = COMPLIANCE_MODEL, onUsage, overrides = '' } = {}) {
  const res = await getClient().messages.create(buildComplianceParams(perFile, { model, overrides }));
