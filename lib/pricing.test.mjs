...
test('estimateCostUsd: fresh input + output at Opus rates', () => {
  // 1M fresh input @ $5 + 1M output @ $25 = $30
  const cost = estimateCostUsd('claude-opus-4-7', {
    input_tokens: 1_000_000,
    output_tokens: 1_000_000,
  });
  assert.ok(Math.abs(cost - 30) < 1e-6, `expected 30, got ${cost}`);
});
...
test('estimateCostUsd: cached reads are ~10% of input price', () => {
  ...
  assert.ok(Math.abs(cost - 0.5) < 1e-6, `expected 0.5, got ${cost}`);
});
...
test('estimateCostUsd: batch option applies the 50% discount', () => {
  ...
  assert.ok(Math.abs(full - 30) < 1e-6, `full expected 30, got ${full}`);
  assert.ok(Math.abs(batched - 15) < 1e-6, `batched expected 15, got ${batched}`);
});
